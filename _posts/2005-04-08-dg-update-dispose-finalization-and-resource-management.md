...
[CBrumme](http://blogs.msdn.com/cbrumme/), [Jeff
Richter](http://www.wintellect.com/weblogs/wintellect/), and a couple other
...
resources—you should give users the ability to explicitly release these them.
...
þ     Do implement the dispose pattern when your type is unsealed and contains
...
þ     Do fully implement the dispose pattern on classes with disposable
...
þ     Do implement IDisposable only on classes where a class in its parent
...
Annotation (Brad Abrams):  We had a fair amount of debate about the relative
ordering of calls in the Dispose() method. E.g.
...
þ     Do create or override the protected virtual void Dispose(bool disposing)
...
þ     Do make a call to your base class's Dispose(bool disposing) method (if
...
ý     Do not re-implement the IDisposable interface, override void Dispose(),
...
ý     Do not create any other variations of the Dispose method other than the
...
·         Introducing a public void Dispose() or protected virtual void
Dispose(bool) method into an existing class hierarchy could cause C# compiler
warnings for subclasses which have introduced disposability themselves, i.e.
...
reimplement with private sealed
...
if base version is not sealed then override public sealed
...
þ     Do implement IDisposable on every type that has a finalizer. This gives
...
þ     Do allow your Dispose method to be called more than once. The method may
...
þ     Do transitively dispose of any disposable fields defined in your type
...
Annotation (Herb Sutter): In C++, you can just follow the natural idiom of
...
example, you'd hold the TextReader by value:
...
Here R::~R() will automatically call tr.~TextReader(), following the usual C++
semantics. (More specifically, in the compiler-generated machinery
R.Dispose(true) will invoke tr.Dispose().)

þ     Consider setting disposed fields to null before actually executing
...
In this example, given an instance of CyclicClassA _a_ and CyclicClassB _b_, if
_a_.cycle = _b_ and _b_.cycle = _a_, transitive disposal would ordinarily cause
...
ý     Avoid throwing an exception from within Dispose except under critical
...
þ     Consider making your object unusable after calling Dispose. Recreating
...
þ     Do implement a Close method for cleanup purposes if such terminology is
...
þ     Consider nulling out large managed object fields in your Dispose method.
...
ý     Avoid creating disposable value types, and never create a value type
...
þ     Consider disposing of any IDisposable object instances when you are done
...
ý     Do not swallow exceptions arising from a call to Dispose. Invoking
...
þ     Do carefully consider any case where you think a finalizer is needed.
...
þ     Do make your Finalize method protected, not public or private. C# and
C++ developers do not need to worry about this, as the compiler does it for you
automatically.

þ     Do free only owned unmanaged resources in your type's Finalize method.
...
For example, a finalizable object _a_ that has a reference to another
finalizable object _b_ cannot reliably use _b_ in _a_'s finalizer, or vice
...
ý     Do not directly call the Finalize method. This is not a legal operation
...
þ     Do gracefully handle situations in which your finalizer is invoked more
...
ý     Do not assume your object is unreachable during finalization. Other
...
ý     Do not assume your finalizer will always run. In some very rare
...
queue), or if a process is terminated without a managed shutdown (e.g. PInvoke
...
þ     Consider using a critical finalizable object (SafeHandle, or any type
...
ý     Avoid allocating memory in finalizers. Allocations may fail due to lack
of memory, and finalizers should be simple enough that they don't fail.

ý     Do not allocate memory from within a critical finalizer, or from
...
ý     Do not call virtual members from finalizers except for in very
...
þ     Do write finalizers that are tolerant of partially constructed instances
...
þ     Do write finalizers that are threading-agnostic. Finalizers can execute
...
ý     Avoid blocking execution from within a finalizer. For example, do not
...
ý     Do not raise unhandled exceptions or otherwise leak exceptions from your
...
ý     Avoid resurrecting yourself by setting a reference to a rooted context,
...
ý     Do not assume that avoiding resurrecting yourself prevents you from
...
ý     Do not modify thread context from within your finalizer, as it will end
...
ý     Do not assume that, because some object is reachable while you're being
...
ý     Do not define finalizers on value types. Only reference types get
...
above doesn't strictly need to add in its own disposed Boolean -- it could
...
þ     Do use SafeHandle for wrapping scarce unmanaged resources such as OS
...
ý     Avoid making calls to GC.Collect() and GC.GetTotalMemory(true) with the
